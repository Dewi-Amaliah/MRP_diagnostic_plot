...
Note: Add pid3 variable in the wrangled data.

Netx week goal 2: 2. Interpete/write paragraph of the result 

1. Base model: age, race_ethnicity, state -> outcome would be future vote (collapsed race_ethnicity)
2. Additonal predictor: education (collapsed race_ethnicity)
3. Model with additional level: all of the race_ethnicity and the collapsed race_ethnicity (work with the ACS data to get the hispanic) if someone answer white+hispanic -> hispanic -> outcome would be future vote 
4. Model 2 with 3 different outcome

- Inspect the time ellapsed to run every method when using mrpkit
