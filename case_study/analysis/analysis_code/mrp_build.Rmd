...
race_ethnicity_pop <- get_ct_acs(race_ethnicity) %>%
  rename(`race and etnicity` = race_ethnicity)
...
knitr::kables(
  list(
    knitr::kable(voted_ct,
                 caption = "For whom did you vote for President of the United States?",
                 col.names = c("response", "%")),
    knitr::kable(intent_ct,
                 caption = "Which candidate did you prefer for President of the United States?",
                 col.names = c("response", "%")),
    knitr::kable(party_id_ct,
                 caption = "Party identity, codes self-identified Independents in pid3 who expressed leaning towards a party in pid7.",
                 col.names = c("response", "%"))),
  caption = "Percentage of each answer in CCES 2016. This question will be the MRP models' outcome in this case study. Since the model is binary, these answer will be converted to be yes/no in the context of vote for Trump/Republican.") %>%
  kable_styling()
```
...
ggplot(state_estimation_data, 
  ...
  scale_fill_gradientn(colors=c("blue","white","red"),
                       ...
                       limits=c(0,1)) +
  ggtitle("MPR estimates of Trump vote share by states") +
  theme_map() +
  labs(fill = "Trump vote share") +
...
```{r scatter-est-truth, fig.width=9, fig.height=8}
get_scatter_common <- function(x, title, subtitle, lower, upper, rmse, mad){
...
spc1 <- get_scatter_common(comparison_data$base_model, 
                           "A. For whom did you vote for President of the US?",
                           "Base model",
...
spc2 <-  get_scatter_common(comparison_data$modelb, 
                           "B. For whom did you vote for President of the US?",
                           "Predictor includes education",
                           comparison_data$lower_modelb,
                           comparison_data$upper_modelb,
                           rmse_modelb,
                           mad_modelb)

spc3 <- get_scatter_common(comparison_data$modelc, 
                           "C. For whom did you vote for President of the US?",
...
spc4a <- get_scatter_common(comparison_data$intent, 
                           "D. Which candidate did you prefer for President?",
                           "Predictor includes education",
...
spc4b <- get_scatter_common(comparison_data$party, 
                           "E. Party identity (including leaners)",
                           "Predictor includes education",
...
spc1 + spc2 + spc3 + spc4a + spc4b + plot_layout(nrow = 2)
```

Doing the other way around for the axis

```{r scatter-est-truth-axis, fig.width=9, fig.height=8}
get_scatter <- function(y, title, subtitle, lower, upper, rmse, mad){
  ggplot(comparison_data, 
         aes(x = truth, 
             y = y, 
             ymin = lower, 
             ymax = upper)) +
  geom_point(color = "#DE0100", size = 1, alpha = 0.6) +
  geom_abline(intercept = 0, linetype = "dotted") +
...
sp1 <- get_scatter(comparison_data$base_model, 
                           "For whom did you vote for President of the US?",
                           "Base model",
                           comparison_data$lower_base_model,
                           comparison_data$upper_base_model,
                           rmse_base,
...
sp4a <- get_scatter(comparison_data$intent, 
                           "Which candidate did you prefer for President?",
                           "Predictor includes education",
                           comparison_data$lower_intent,
                           comparison_data$upper_intent,
                           rmse_intent,
...
sp1 + sp4a + sp4b + sp2 + sp3 + plot_layout(nrow = 2)
```


# plot of state against CI
...
# # est_method_data
# 
# est_method_data <- rbind(raw_est, wtd_estimates, mrp_estimates, truth) %>%
#   mutate(est_method = factor(est_method, levels = c("truth", "raw", "wtd", "mrp")))

```

Scatter plot of each estimation method compared to truth
...
#set the shape and color for color-blind friendly (gg something color-blind)
```
...
compare_method_df %>%
  ...
  ggplot(aes(x = deviance, y = state, color = est_method)) +
  geom_vline(xintercept = 0) +
  geom_point() +
  theme_bw() +
  theme(legend.position = "bottom") +
  xlab("Deviance from the actual value of probability vote for Trump") +
  ylab("State")

# change the color to be color-blind friendly
# gives uncertainty to the geom-point (give notation of actual - estimates) and the measurement unit
# the more conservative the state, the higher the errors 
```

# Violin plot (displaying the effect of using different predictors and levels)
...
```{r violin-levels, fig.width=8, fig.height=7}
...
p6 <- ggplot(comp_race, aes(x = original_re, y = value, fill = model)) +
  ...
  ggtitle("Trump's vote share in the 2016 presidential vote by race") +
...
p7 <- ggplot(comp_educ, aes(x = education, y = value, fill = model)) +
  ...
  theme(axis.title = element_text(size = 9),
        ...
        legend.title = element_blank(),
        legend.position = "bottom") +
  ggtitle("Trump's vote share in the 2016 presidential vote by education") +
...
p8 <- ggplot(comp_age, aes(x = age, y = value, fill = model)) +
  ...
  theme(axis.title = element_text(size = 9),
        ...
        legend.title = element_blank(),
        legend.position = "bottom") +
  ggtitle("Trump's vote share in the 2016 presidential vote by age") +
...
cp1 <- ggplot(com_outcome, aes(x = correlation, y = model)) +
...
cp2 <- ggplot(com_predictor, aes(x = correlation, y = model)) +
  geom_point(size = 3, color = "red") +
  ...
  annotate("text", x = com_predictor$correlation[2], 
           ...
           vjust = -1) +
  ggtitle("Correlation between models with different predictors (same outcome)")
```

```{r}
cp1 + cp2 + plot_layout(nrow = 2)
```


Note:

- Correlation: 1-correlation (since higher correlation is better)
- Remove bias from the plot note that mae and bias are the same thing
- There is only one column, so facet is ncol=1, nrow=4
- Order the data in terms of the value so we can see wether the group/level changing
- Add credible interval length (upper-lower), the narrower the better 
- For the posterior estimates, use median instead of mean. 
- Compare raw vs weighted vs MRP in model with education with states as the level 
...
# get_MRP_ests_ver2 <- function(est_data, var){
#   tmp <- est_data %>%
#   pivot_longer(as.character(1:4000),
#                names_to = "posterior_draw",
#                values_to = "posterior_sample") %>%
#   group_by({{var}}, posterior_draw) %>% # Add extra grouping vars here
#   summarise(post_strat_ests = sum(posterior_sample*N_j)/sum(N_j))
#   
#   return_df <- tmp %>%
#   group_by({{var}}) %>%
...
# mrp_race_state_fit3_ver2 <- get_MRP_ests_ver2(ests_ps_fit1, original_re)
```


```{r}
ests_ps_fit1 <- cbind(map_model$poststrat_data(), fit1_predict)

est_fit1_by_race <- ests_ps_fit1 %>%
  pivot_longer(as.character(1:4000),
               names_to = "posterior_draw",
               values_to = "posterior_sample") %>%
  group_by(original_re, posterior_draw) %>%
  summarise(post_strat_ests = sum(posterior_sample*N_j)/sum(N_j))


sum_table <- est_fit1_by_race %>%
...
# check with aggregate method from mrpkit
sum_table_ck <- fit1$aggregate(fit1_predict, by = "original_re") %>%
  group_by(original_re) %>%
...
# Compare Black category, for example
...
black <- ests_ps_fit1 %>%
  ...
  group_by(original_re, posterior_draw) %>%
  summarise(post_strat_ests = sum(posterior_sample*N_j)/sum(N_j)) %>%
  filter(original_re == "Black")

# the aggregate method
black_ck <- fit1$aggregate(fit1_predict, by = "original_re") %>%
  filter(original_re == "Black")
```
...
**Metrics of race between base model and model with more levels on race**
...
race_metrics %>%
  group_by(metrics) %>%
  ungroup %>%
  mutate(metrics = as.factor(metrics),
         level = reorder_within(level, values, metrics)) %>%
  ggplot(aes(level, values, color = level)) +
    geom_point(show.legend = FALSE) +
    facet_wrap(~metrics, scales = "free", nrow = 3) +
    coord_flip() +
    scale_x_reordered() +
...
```{r cor-race-plot, fig.width=5, fig.height=6}
...
```{r metrcs-age-fit2, fig.width=5, fig.height=6}

get_metrics_plot(age_metrics,
                 "Estimation performance criteria by age level",
                 "Model with additional predictor (education) compared to base model",
                 "Age levels")
```
...
```{r metrics-age-fit3, fig.width=5, fig.height=6}

get_metrics_plot(age_metrics2,
                 "Estimation performance criteria by age level",
                 "Model with more levels on race compared to base model",
                 "Age levels")
```
# Compare the length of credible interval


```{r}
# function to get the plot

metrics_ci_label <- c(`mean_ratio` = "Mean of CI length ratio",
                   `mean_ci_dev` = "Mean of CI length difference")
...
get_ci_plot <- function(data, level, title, subtitle, xlab){
  ...
  facet_wrap(~metrics, scales = "free", ncol = 2,
             labeller = as_labeller(metrics_ci_label)) +
  scale_x_discrete(labels = function(x) gsub("__.+$", "", x)) +
  coord_flip() +
  scale_colour_brewer(palette = "Dark2") +
  theme_bw() +
...
1. Write clearly that the model that the comparison is between 2 models, and not compare to ground truth, so we can not interpret RMSE as the way we used to intreprete it. 
2. Make separate plots for each metrics, so there is no misinterpretation abou the scale direction. 
