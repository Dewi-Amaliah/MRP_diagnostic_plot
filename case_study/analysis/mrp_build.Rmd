...
get_mad <- function(vect){
  paste0("RMSE= ", round(mae(comparison_data$truth, vect), 4))
}
...
ggplot(est_method_data, aes(x = mean, y = state, xmin = lower, xmax = upper,
                            color = est_method)) +
  geom_point(position = position_dodgev(height = .5)) +
  geom_errorbarh(position = position_dodgev(height = .5),
                 alpha = 0.7) +
  theme_bw() +
  scale_color_manual(values = c("black", "magenta", "blue", "tomato")) +
  xlab("Probability of voting Trump") +
  ggtitle("Probability of voting Trump with different estimation method")
```
...
get_metrics <- function(vect, upper, lower){
  ...
  ci_len <- mean(upper) - mean(lower)
  
  
  metrics <- tribble(~level, ~metrics, ~values,
                    "Total", "rmse", rmse,
                    "Total", "cor_adj", cor_adj,
                    "Total", "mae", mae,
                    "Total", "ci_len", ci_len) %>%
...
race_metrics_tmp <- left_join(mrp_race_state_fit1, mrp_race_state_fit3, by = c("state", "original_re"), 
  ...
  summarise(cor_adj = 1 - cor(median_fit1, median_fit3),
            ...
            ci_len = mean(ci_len_fit3)) %>%
  pivot_longer(c(2:5), names_to = "metrics", values_to = "values") %>%
...
metrics_label <- c(`cor_adj` = "Adj. correlation",
                   `rmse` = "RMSE",
                   `mae` = "MAE",
                   `ci_len` = "Length of CI")

get_metrics_plot <- function(data, title, subtitle){
  plot <- data %>%
  ...
  geom_segment(aes(xend = level, yend = 0), 
               colour = "grey50") +
  geom_point(size = 2, aes(colour = metrics)) + 
  ggtitle(title,
          subtitle = subtitle) +
  facet_grid(metrics ~., scales = "free", space = 'free',
