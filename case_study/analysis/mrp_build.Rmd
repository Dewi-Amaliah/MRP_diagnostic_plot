...
est_method_data <- rbind(raw_est, wtd_estimates, mrp_estimates)
```
...
```{r cor-race}
# in total 
get_metrics <- function(vect){
  cor <- cor(comparison_data$base_model, vect)
  mae <- mae(comparison_data$base_model, vect)
  rmse <- rmse(comparison_data$base_model, vect)
  bias <- bias(comparison_data$base_model, vect)
...
race_metrics <- rbind(race_metrics_tmp, metrics_fit1_fit3) %>%
  mutate(metrics = factor(metrics, levels = c("cor", "rmse", "mae", "bias")),
         level = factor(level, levels = c("Black", "White", "Hispanic", "Asian", "Native American",
                                          "All Other", "total")))


ggplot(race_metrics, aes(y = level, x = values)) +
  geom_point(alpha = 0.8) +
  scale_color_manual(values = c("#7fc97f", "#beaed4", "#fdc086", "#ffff99",
                                "#386cb0", "#f0027f", "#616A6B")) +
  ggtitle("Estimation performance criteria by race categories",
          subtitle = "Model with more levels of race compared to base model") +
  theme_bw() +
...
ggplot(educ_metrics, aes(y = level, x = values)) +
  geom_point() +
  scale_color_manual(values = c("#66c2a5", "#fc8d62", "#8da0cb", "#e78ac3", "#616A6B")) +
  ggtitle("Estimation performance criteria by education level",
          subtitle = "Model with additional predictor (education) compared to base model") +
  theme_bw() +
  theme(plot.title = element_text(size = 12),
        plot.subtitle = element_text(size = 9)) +
  xlab("values") +
...
ggplot(age_metrics, aes(y = level, x = values)) +
  geom_point() +
  scale_color_manual(values = c("#fcc5c0", "#fa9fb5", "#f768a1", "#c51b8a", "#7a0177","#616A6B")) +
  ggtitle("Estimation performance criteria by age level",
          subtitle = "Model with additional predictor (education) compared to base model") +
  theme_bw() +
  theme(plot.title = element_text(size = 12),
        plot.subtitle = element_text(size = 9)) +
...
ggplot(age_metrics2, aes(y = level, x = values)) +
  geom_point() +
  scale_color_manual(values = c("#fcc5c0", "#fa9fb5", "#f768a1", "#c51b8a", "#7a0177","#616A6B")) +
  ggtitle("Estimation performance criteria by age level",
          subtitle = "Model with more levels on race compared to base model") +
  theme_bw() +
  theme(plot.title = element_text(size = 12),
        plot.subtitle = element_text(size = 9)) +
  xlab("values") +
...
cces_mrp_na_omit <- cces_mrp %>%
  filter(!is.na(vote))
wtd_est <- svydesign(ids = ~1, weights = ~wt, data = cces_mrp_na_omit)
wtd_mean_table <- confint(svyby(~vote, ~state, wtd_est, svymean))

ymax = wtd_mean_table$mean[2] + 1.96*wtd_mean_table$SE[2]
ymin = wtd_mean_table$mean[2] - 1.96*wtd_mean_table$SE[2]
```
